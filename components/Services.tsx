...
const Services = () => {
  return (
    <section className="my-36 bg-gray-900 py-16">
      <div className="mx-auto max-w-6xl text-center">
        <h2 className="mb-8 text-4xl font-bold uppercase leading-snug tracking-wide text-blue-100">Our Services</h2>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {services.map((service, index) => (
            <div
              key={index}
              className="flex flex-col rounded-lg bg-gray-800 p-6 shadow-md transition hover:shadow-lg" // Added flex-col for vertical alignment
            >
              ...
              {service.image && (
                <div className="relative mb-4 flex h-48 w-full items-center justify-center">
                  ...
                  <img
                    src={service.image}
                    alt={service.title}
                    className="h-full max-h-full w-auto" // Maintain aspect ratio with max height
                  />
                </div>
              )}
